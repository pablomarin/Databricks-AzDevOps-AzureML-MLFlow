...
def create_cluster():
    ...
    "spark_env_vars":
        {
        "TENANT_ID": os.environ['SVCDirectoryID'],
        "SERVICE_PRINCIPAL_ID": os.environ['SVCApplicationID'],
        "SERVICE_PRINCIPAL_SECRET": os.environ['SVCSecretKey'],
        "SUBSCRIPTION_ID": os.environ['DBRKS_SUBSCRIPTION_ID'],
        "AZML_WORKSPACE_NAME": os.environ['AzmlWORKSPACEname'],
        "AZML_RESOURCE_GROUP": os.environ['AzmlResourceGroup']
        }
    }
    
    

    response = requests.post("https://"+os.environ['DBRKS_INSTANCE']+".azuredatabricks.net/" + DBRKS_START_ENDPOINT, headers=DBRKS_REQ_HEADERS, json=postjson)
